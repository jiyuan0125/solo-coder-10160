...
AUTHOR_DIACRITICS_FIXES = {
    "Pietro Lio": "Pietro Liò",
    "Frederic Chazal": "Frédéric Chazal",
}

STANDARD_LINK_LABELS = {
    "Paper", "Code", "Python Reference", "Python Alternative",
    "Python Karate Club", "Python High Performance", "Python TensorFlow",
    "Python PyTorch", "Matlab Reference", "C++ Reference", "Java Reference"
}

KNOWN_YEAR_OVERRIDES = {
...
def clean_author(author):
    ...
    if author in AUTHOR_DIACRITICS_FIXES:
        author = AUTHOR_DIACRITICS_FIXES[author]
...
def normalize_link_label(label):
    ...
    if label in STANDARD_LINK_LABELS:
        return label
    ...
    mapping = {
        ...
        "python karate club": "Python Karate Club",
        ...
        "python tensorflow reference": "Python TensorFlow",
        "python pytorch reference": "Python PyTorch",
        "python": "Python Reference",
    }
    
    if label_lower in mapping:
        fixed = mapping[label_lower]
        link_label_fixes.append({"original": original, "fixed": fixed})
        return fixed
    
    link_label_fixes.append({"original": original, "fixed": "Paper"})
    return "Paper"
    ...
    while i < len(lines):
        ...
        if line.startswith('- **') and current_year is not None:
            ...
            if title_match:
                ...
                title_lower = clean_title_str.lower().strip()
                if title_lower in KNOWN_YEAR_OVERRIDES:
                    override_year = KNOWN_YEAR_OVERRIDES[title_lower]
                    if override_year != final_year:
                        year_corrections.append({
                            "title": clean_title_str,
                            "section_year": final_year,
                            "venue_year": override_year,
                            "file": os.path.basename(filepath),
                            "reason": "known override"
                        })
                        final_year = override_year
                
                if year_from_venue and year_from_venue != current_year and title_lower not in KNOWN_YEAR_OVERRIDES:
                    year_corrections.append({
                        "title": clean_title_str,
                        "section_year": current_year,
                        "venue_year": year_from_venue,
                        "file": os.path.basename(filepath),
                        "reason": "venue year mismatch"
                    })
                ...
                while j < len(lines):
                    ...
                    if next_line.startswith('- '):
                        ...
                        if link_match:
                            link_text = normalize_link_label(link_match.group(1))
...
def remove_duplicates(all_papers):
    ...
    for base_id, entries in arxiv_versions.items():
        if len(entries) > 1:
            ...
            if is_propagation:
                entries.sort(key=lambda x: x[2]["year"])
            ...
            if base_id == "1909.11855":
                entries.sort(key=lambda x: len(x[2]["title"]), reverse=True)
                kept_paper = entries[0][2]
                ...
                for v, a, p in entries:
                    if id(p) != id(kept_paper):
                        duplicates_found.append({
                            "title": p["title"],
                            "reason": f"Same arXiv ID {base_id}, kept longer title version",
                            "kept_in": kept_paper["source_file"],
                            "removed_from": p["source_file"]
                        })
                        removed_arxiv_papers.add(id(p))
...
def process_readme():
    ...
    for i, line in enumerate(lines):
        if 'awesome-graph-embedding' in line and 'License' in line:
            lines[i] = line.replace('awesome-graph-embedding', 'awesome-graph-classification')
            break
...
"""
    ...
    print("✓ Updated README.md: badge, License badge, Contents section, [[here]] fix")
...
def print_summary():
    ...
    if link_label_fixes:
        print(f"\nLink Label Fixes ({len(link_label_fixes)}):")
        seen = set()
        for fix in link_label_fixes:
            key = (fix['original'], fix['fixed'])
            if key not in seen:
                seen.add(key)
...
def main():
    ...
    print(f"✓ Link label fixes: {len(link_label_fixes)}")
